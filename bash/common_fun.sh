...
function mipi_disable () {
    echo "mipi_disable"
    $tool_path/serdes_cfg $dev 100 "i2ctransfer -f -y 0 w3@0x48 0x03 0x13 0x00"  >> $std_out
...
function i2ctransfe_fun () {
    channel=$1
    value=$2
    echo "channel:" $channel
    if [ $value -eq 0 ] 
        then
        #echo "i2ctransfer -f -y $channel w3@0x48 0x03 0x13 0x42"96705
        $tool_path/serdes_cfg $dev 100 "i2ctransfer -f -y $channel w3@0x48 0x03 0x13 0x42" >> $std_out
        else
        #echo "i2ctransfer -f -y $channel w3@0x48 0x03 0x13 0x2" 9295
        $tool_path/serdes_cfg $dev 100 "i2ctransfer -f -y $channel w3@0x48 0x03 0x13 0x02" >> $std_out
    fi
}
