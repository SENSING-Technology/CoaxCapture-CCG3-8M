...
uint32_t send_cmd_to_a53_sync(const char send_cmd[])
{
    ...
    do
    {
        //printf("%s cursor:%d, cmd_length:%d\n",  __func__, cursor, cmd_length);
        if (cursor + 4 <= cmd_length)
        {
            word = 0;
            memcpy((void *)&word, (const void *)send_cmd + cursor, 4);
            //printf("%s word:%d, cmd_length:%d\n",  __func__, word, cmd_length+cursor);
    ...
    while (loop_count < 1000)
    {
		...
		loop_count += 1;
		//printf("%s(%d) Interrupt result:%d, loop_count:%d\n", __func__, __LINE__, result, loop_count);
    }
	//printf("loop_count:%d\n", loop_count);
    if (loop_count == 1000 && (result != 0))
    {
        ...
        printf("%s(%d) Interrupt state is error, A53 malfunction \n", __func__, __LINE__);
        return -1;
    }
