...
void element_channel_process(void *BaseAddress, uint32_t channel, DATA_ELEMENT *elem, int elem_num)
{
	...
	for (i = 0; i < elem_num; i++)
	{
		...
		if (bits == 0x10)
		{
			...
			reg_l = reg & 0xff;
			memset(cmd, 0, 256);
			sprintf(cmd, "i2ctransfer -f -y %d w3@0x%02x 0x%02x 0x%02x 0x%02x\n", channel, dev, reg_h, reg_l, data);
		}
		else if (bits == 0x08)
		{
			reg_l = reg & 0xff;
...
void element_channel_process_data(void *BaseAddress, uint32_t channel, DATA_ELEMENT *elem, int elem_num)
{
	int i;
	uint8_t dev;
	uint16_t reg;
