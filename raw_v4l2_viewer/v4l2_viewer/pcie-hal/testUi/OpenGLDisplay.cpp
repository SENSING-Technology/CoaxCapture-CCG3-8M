...
void OpenGLDisplay::DisplayVideoFrame(unsigned char *data, int frameWidth, int frameHeight)
{
    ...
    unsigned char* pdesy = (unsigned char*)impl->mBufYuv;
    unsigned char* pdesu = (unsigned char*)impl->mBufYuv + frameWidth*frameHeight;
    unsigned char* pdesv = (unsigned char*)impl->mBufYuv + frameWidth*frameHeight*3/2;
...
//    std::cout<<"frameWidth: "<<frameWidth<<"frameHeight: "<<frameHeight<<std::endl;
    for(j = 0; j < frameHeight; j++){
            ...
            pdesu += (frameWidth/2);
            pdesv += (frameWidth/2);
            for(i = 0; i < frameWidth*2; i+=4){
                pdesy[i/2] = psrc[i+1];
                pdesu[i/4] = psrc[i+0];
                pdesy[i/2+1] = psrc[i+3];
                pdesv[i/4] = psrc[i+2];
            }
    }
#else
...
void OpenGLDisplay::paintGL()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, impl->mVideoW/2, impl->mVideoH
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, impl->mVideoW/2, impl->mVideoH
                 , 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, (char*)impl->mBufYuv + impl->mVideoW * impl->mVideoH * 3/2);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
/*
 * OPENGL 提取RGB数据： https://blog.csdn.net/niu2212035673/article/details/80251949
 *
 使用方法
